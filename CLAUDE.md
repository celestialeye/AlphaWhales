...
- Select historical filings by report date, not filing date. Reconstruct each period in filing chronology: originals and explicit restatements replace the base, and later NEW HOLDINGS amendments supplement it. Retain effective accessions and reject unknown amendment types or missing bases. Normalize legacy values from implied per-share values and rebuild totals/weights before comparing quarters. Transient failures are retryable errors; a confirmed missing filing is unknown, not liquidation.
...
- The QoQ Signals view uses a Sector × Action matrix with notable/total position counts. Keep `UNCHANGED` framed as unchanged reported shares rather than a recommendation. Resolve sector and industry from cached ticker profiles first and `data/reference/full_universe.csv` second, without issuing market requests during period-view construction; expose missing coverage as `Unclassified`.
