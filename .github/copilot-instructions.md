...
- Alpha Whale Sentiment requires at least three directional managers. Breadth is the normalized balance of `NEW`/`INCREASED` versus `DECREASED`/`CLOSED`; conviction is the normalized balance of actual portfolio-weight changes for directional share-action observations using an expanding 95th-percentile cap. Share-action and weight-change signs may disagree. Composite score is an equal 50/50 average bounded to `[-100, 100]`.
...
- The sentiment heatmap and contributor lists must use the same directional-action weight-change field as the conviction calculation. Contributor headline values use clipped changes and may show actual changes as secondary context.
