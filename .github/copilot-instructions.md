...
- Alpha Whale Sentiment keeps raw share activity separate from scored conviction. Estimated trade weight is `shares_change * quarter_average_price / prior_portfolio_value`; relative conviction divides that by the manager's median prior position weight. Trades below `0.25x` typical are routine, scored contributions cap at `2x`, and at least three meaningful managers are required.
...
- The sentiment heatmap and contributor lists must use manager-relative conviction. Contributor headline values use the capped multiple used in scoring and show estimated trade weight as secondary context.
